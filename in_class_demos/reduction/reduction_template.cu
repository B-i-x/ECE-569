...
#define CUDA_CHECK(call)                                                              \
    do {                                                                              \
        ...
        if (cudaSuccess != err) {                                                     \
            ...
            if (fp) {                                                               \
                fprintf(fp, "CUDA error in file '%s' at line %i: %s.\n", __FILE__, __LINE__, \
                        cudaGetErrorString(err));                                     \
                fclose(fp);                                                         \
            }                                                                         \
            exit(EXIT_FAILURE);                                                       \
...
int main(int argc, char **argv) {
    FILE *logfp = fopen("debug_log.txt", "w");
    if (logfp) {
    ...
    CUDA_CHECK(cudaMalloc((void **) &d_intermediate, ARRAY_BYTES));
    ...
    // (Assume your kernels and reduce() function are defined above this main.)
    ...
    // Run reduction kernels (assuming your reduce() function with CUDA checks is defined).
    for (int whichKernel = 0; whichKernel < 5; whichKernel++) {
        printf("===========================================\n");
        ...
        switch (whichKernel) {
            case 0:
                ...
                for (int i = 0; i < 100; i++) {
                    // Call your reduce() function; ensure it has CUDA_CHECK within.
                    // For example: reduce(d_out, d_intermediate, d_in, ARRAY_SIZE, 0);
                }
                CUDA_CHECK(cudaEventRecord(stop, 0));
                break;
            // Add cases 1-4 similarly...
            default:
